...
import { Power, Sun, BatteryCharging, PowerOff, List, Trash2, Plus, Info, Printer, Briefcase, ArrowLeft, Edit, Save, Moon, SunDim, AlertTriangle, CheckCircle, ArrowRight } from 'lucide-react';
        ...
        <div className="w-full max-w-4xl mx-auto py-8">
            ...
            <div className="mb-8 print-hidden">
                <div className="flex justify-between mb-2">
                    {steps.map(step => (
                        <div key={step.number} className={`text-xs text-center flex-1 ${currentStep >= step.number ? 'text-blue-600 dark:text-blue-400 font-bold' : 'text-gray-400 dark:text-gray-500'}`}>
                            {step.title}
                        </div>
                    ))}
                </div>
            ...
            <div className="mt-8 flex justify-between items-center print-hidden">
                <button 
                    onClick={handleBack} 
                    className={`flex items-center space-x-2 px-6 py-3 rounded-lg font-semibold transition-all duration-300 ${currentStep > 1 ? 'bg-gray-200 dark:bg-gray-700 hover:bg-gray-300 dark:hover:bg-gray-600' : 'opacity-0 cursor-not-allowed'}`}
                ...
                <button 
                    onClick={handleNext} 
                    className={`flex items-center space-x-2 px-6 py-3 rounded-lg font-semibold text-white transition-all duration-300 ${currentStep < TOTAL_STEPS ? 'bg-blue-600 hover:bg-blue-700 shadow-lg' : 'opacity-0 cursor-not-allowed'} ${!isStepValid ? 'bg-gray-400 dark:bg-gray-600 cursor-not-allowed' : ''}`}
